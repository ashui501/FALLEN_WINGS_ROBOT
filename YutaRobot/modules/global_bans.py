...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in DEV_USERS:
        message.reply_text(
            "That user is member of Eldian Arena\nI can't act against our own.",
        )
        return
    ...
    if int(user_id) in DEMONS:
        message.reply_text(
            "OOOH someone's trying to gban a Surveycorp! *grabs popcorn*",
        )
        return

    if int(user_id) in TIGERS:
        message.reply_text("That's a Eldian! They cannot be banned!")
        return

    if int(user_id) in WOLVES:
        message.reply_text("That's a Marleyan! They cannot be banned!")
        return
...
def gbanstat(update: Update, context: CallbackContext):
    ...
    if len(args) > 0:
        if args[0].lower() in ["on", "yes"]:
            ...
            update.effective_message.reply_text(
                "Antispam and spamwatch are now enabled ✅\n"
                "I am now protecting your group from potential remote threats!",
            )
        elif args[0].lower() in ["off", "no"]:
            sql.disable_gbans(update.effective_chat.id)
            update.effective_message.reply_text(
                "Antispam and spamwatch are now disabled ❌",
            )
    else:
        update.effective_message.reply_text(
...
*Note:* Users can appeal gbans or report spammers at @samurai_botsupport
