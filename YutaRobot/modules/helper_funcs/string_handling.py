...
BTN_URL_REGEX = re.compile(r"(\[([^\[]+?)\]\((buttonurl:(?:/{0,2})(.+?)|rulesbtn)(:same)?\))")
...
def markdown_parser(
    ...
    for ent, ent_text in entities.items():
        ...
        if ent.type in ("code", "url", "text_link"):
            ...
            if ent.type == "url":
                if any(
                    match.start(0) <= start and end <= match.end(0)
...
def button_markdown_parser(
    txt: str, entities: Dict[MessageEntity, str] = None, offset: int = 0, chat_id: Optional[int] = None,
) -> (str, List):
    ...
    for match in BTN_URL_REGEX.finditer(markdown_note):
        ...
        if n_escapes % 2 == 0:
            link = match.group(3)
            if link == 'rulesbtn' and chat_id is not None:
                link = f'https://t.me/{updater.bot.username}?start={chat_id}'
            else:
                link = match.group(4)
            # create a thruple with button label, url, and newline status
            buttons.append((match.group(2), link, bool(match.group(5))))
